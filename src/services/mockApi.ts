...
export const mockApi = {
  ...
  async getOfficePermissions(officeId: string) {
    await new Promise((res) => setTimeout(res, 150));
    const config = getStoredData<Record<string, string[]>>("apg_office_modules", INITIAL_OFFICE_MODULES);
    return config[officeId] || [];
  },

  async saveOfficePermissions(officeId: string, permissionKeys: string[]) {
  ...
  async addUser(userData: Record<string, any>) {
  ...
  async getOfficeModules(officeId: string = "ent-1") {
    await new Promise((res) => setTimeout(res, 150));
    const config = getStoredData<Record<string, string[]>>("apg_office_modules", INITIAL_OFFICE_MODULES);
    return config[officeId] || [];
  },

  async updateOfficeModules(officeId: string, activeModuleIds: string[]) {
    await new Promise((res) => setTimeout(res, 250));
    const config = getStoredData<Record<string, string[]>>("apg_office_modules", INITIAL_OFFICE_MODULES);
  ...
  async getRoleSubModules(roleCode: string = "SUPER_ADMIN") {
    await new Promise((res) => setTimeout(res, 150));
    const config = getStoredData<Record<string, string[]>>("apg_role_submodules", INITIAL_ROLE_SUBMODULES);
    return config[roleCode] || [];
  },

  async updateRoleSubModules(roleCode: string, assignedSubModuleIds: string[]) {
    await new Promise((res) => setTimeout(res, 250));
    const config = getStoredData<Record<string, string[]>>("apg_role_submodules", INITIAL_ROLE_SUBMODULES);
  ...
  async getEffectivePermissions(officeId: string, roleCode: string) {
    await new Promise((res) => setTimeout(res, 200));
    const officeModulesConfig = getStoredData<Record<string, string[]>>("apg_office_modules", INITIAL_OFFICE_MODULES);
    const roleSubModulesConfig = getStoredData<Record<string, string[]>>("apg_role_submodules", INITIAL_ROLE_SUBMODULES);
    ...
    const availableSubModulesInOffice: any[] = [];
  ...
  // Auth API
  async login(username: string, _password: string, rolePreset: string | null = null) {
  ...
  async addRole(roleData: Record<string, any>) {
    await new Promise((res) => setTimeout(res, 300));
    const roles = getStoredData("apg_roles", INITIAL_ROLES);
    const newRole = {
      id: `role-${Date.now()}`,
  ...
  async addEntity(entityData: Record<string, any>) {
    await new Promise((res) => setTimeout(res, 300));
  ...
  async addCOA(accountData: Record<string, any>) {
    await new Promise((res) => setTimeout(res, 300));
  ...
  async processApproval(id: string, _action: string) {
    await new Promise((res) => setTimeout(res, 300));
