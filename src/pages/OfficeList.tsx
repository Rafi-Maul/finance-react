...
interface Office {
  ...
  name: string;
  type: string;
  parent?: string | { name?: string; code?: string } | null;
  reportAccess?: string;
  status?: string;
  badgeType?: string;
  typeClass?: string;
...
interface OfficeListProps {
  onNavigateToPermissions?: (officeId: string) => void;
  onNavigateToEdit?: (officeId: string) => void;
}
...
export const OfficeList = ({ onNavigateToPermissions, onNavigateToEdit }: OfficeListProps) => {
  ...
  const handleAddOffice = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
  ...
  const filteredOffices = offices.filter((o: Office) => {
