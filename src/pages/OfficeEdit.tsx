...
export const OfficeEdit = ({ officeId, onDone }: OfficeEditProps) => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setOffices(officeList.filter((o: OfficeOption) => o.id !== officeId));
        setForm({
          code: office.code || "",
          name: office.name || "",
        ...
        setError(err instanceof Error ? err.message : "Gagal memuat data office");
      } finally {
        setLoading(false);
      }
    };
