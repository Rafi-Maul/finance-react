...
import { INITIAL_OFFICE_MODULES } from "../../../services/mockData";
...
export const ThreeTierAccessManager = () => {
  ...
  const loadOfficeAndRoleData = async (officeId, roleCode) => {
  ...
  // Level 1: Toggle Module for Office
  const toggleOfficeModule = (moduleId) => {
    let updated;
  ...
  // Level 3: Toggle SubModule for Role (Only allowed if parent module is active in Office!)
  const toggleRoleSubModule = (subModuleId, parentModuleId) => {
    if (!officeModules.includes(parentModuleId)) {
      return; // Cannot pick submodule if parent module is inactive in Office
    }
    let updated;
  ...
  // Level 2 Validation: Purge/Sanitize SubModules whose parent module is inactive in Office
  const getSanitizedRoleSubModules = () => {
    const allowedSubModuleIds = [];
  ...
  const handleSaveAll = async () => {
    setSaving(true);
    // Sanitize submodules before saving (Level 2 requirement)
          ...
          <div className="bg-white rounded-2xl p-5 shadow-xs border border-slate-100 space-y-4">
            ...
            <div className="space-y-2">
              {entities.map((ent) => {
                ...
                const activeModCount = (INITIAL_OFFICE_MODULES[ent.id] || []).length;
                return (
                  <button
                    key={ent.id}
              ...
              {systemModules.map((mod) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Level 2 & 3: SubModule Picker (Strictly filtered if Parent Module is Inactive) */}
                ...
                <div className="p-4 bg-emerald-50/80 border border-emerald-200 rounded-2xl text-xs space-y-2">
                  <div className="flex items-center justify-between font-bold text-emerald-900">
                    ...
                    <span className="text-sm bg-[#00c885] text-white px-2.5 py-0.5 rounded-full">
                      {effectiveData.effectiveAllowedSubModules.length} SubModul
                    </span>
                  </div>

                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {effectiveData.effectiveAllowedSubModules.map((sub) => (
