...
type SystemSubModule = SystemModule["subModules"][number];
...
export const RolePermissionEditor = ({ selectedRoleCode = "SUPER_ADMIN", onBack }: RolePermissionEditorProps) => {
  ...
  const [selectionMode, setSelectionMode] = useState<"USER" | "ROLE">("USER");
  ...
  // When Office changes, update filtered users & refresh permissions
  useEffect(() => {
  ...
  // When User Selection changes in USER mode
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      // Filter initial users for default office
  ...
  // Helper to normalize permissions (handles both subModuleId and subModuleId:perm)
  const normalizePermissions = (rawPerms: string[]): string[] => {
    const normalized = new Set<string>();
    SYSTEM_MODULES.forEach((mod) => {
      mod.subModules.forEach((sub) => {
        const perms = sub.permissions || ["Lihat", "Tambah", "Edit", "Hapus"];
        // If rawPerms has full subModule ID, add all its permissions
        if (rawPerms.includes(sub.id)) {
          perms.forEach((p) => normalized.add(`${sub.id}:${p}`));
        } else {
          // Add specific action keys
          perms.forEach((p) => {
  ...
  const loadOfficeAndRolePermissions = async (officeId: string, roleCode: string) => {
    try {
  ...
  const isActionSelected = (subId: string, perm: string) => {
    return selectedPermissions.includes(`${subId}:${perm}`);
  };

  // Toggle individual action (e.g., "SUB_MULTI_CABANG:Hapus")
  const toggleSingleAction = (subId: string, perm: string, parentModuleId: string) => {
  ...
  // Toggle all actions of a single submodule (Rightmost Checkbox)
  const toggleSubModuleAllActions = (subObj: SystemSubModule, parentModuleId: string) => {
    if (!officeModules.includes(parentModuleId)) return;
  ...
  // Toggle all actions inside an entire module
  const toggleModuleAllActions = (moduleObj: SystemModule) => {
    if (!officeModules.includes(moduleObj.id)) return;

    const allModuleActionKeys: string[] = [];
  ...
  // Select all available actions across all active office modules
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      // Purge actions for inactive office modules
  ...
  // Calculations for progress bar & counters (Action level)
  ...
  const getActiveBadgeClass = (permName: string) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        {/* Right Card: Daftar Permission per Modul (Granular Row Layout) */}
        <div className="lg:col-span-8 space-y-4">
          ...
          <div className="space-y-4">
            {SYSTEM_MODULES.map((mod) => {
              ...
              // Calculate total & selected actions in this entire module
              const allModuleActions: string[] = [];
              mod.subModules.forEach((sub) => {
                const perms = sub.permissions || ["Lihat", "Tambah", "Edit", "Hapus"];
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4 flex items-center justify-between bg-white border-b border-slate-100">
                    <div className="flex items-center gap-3">
                      {/* Checkbox Header (Select All Actions in Entire Module) */}
                      <button
                        type="button"
                  ...
                  {/* Module Content (Vertical List: Row per Sub-Module) */}
                  {isExpanded && (
                          ...
                          {mod.subModules.map((sub) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {/* Column 2: 1 per 1 Interactive Action Badges */}
                                ...
                                {/* Column 3 (Ujung Kanan): Checklist Isi Semua Aksi Sub-Modul Ini */}
