...
export const PersediaanModule = ({ activeSubTab = "persediaan/permintaan-barang", onSubTabChange }: OperationalModuleProps) => {
  ...
  const [, setShowAddTrfModal] = useState(false);
  ...
  const [, setShowAddFulModal] = useState(false);
  ...
  const handleAddRequest = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
  ...
  const handleAddItemService = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
  ...
  const handleAddBrand = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
